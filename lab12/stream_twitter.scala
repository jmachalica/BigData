...
for(stream<- spark.streams.active) stream.stop()
...
val najpopularniejszeHashtagiDF = analizaDF.select( explode( $"hashTags").as("hashTag"), lower($"hashTag")).groupBy($"hashTag").count().orderBy($"count").limit(30)
...
val nazwaQuery = najpopularniejszeHashtagiDF .writeStream .format("memory").outputMode("complete").queryName("query_hashtag").  trigger(Trigger.ProcessingTime("10 seconds")).start()
